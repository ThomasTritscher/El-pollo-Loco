...
function draw() {
        ...
        drawAllEnemies();
        drawCharacter();
        drawObject("life-bar", 580, 0, 0.35, 0.15);
    
        drawObject("bottle-icon", 0, 0, 0.1, 0.15);
        drawText("x" + collectableObject["bottle1"].count, "50px Bradley Hand ITC", 65, 60);
    
        drawObject("coin-icon", 120, 5, 0.08, 0.15);
        drawText("x" + collectableObject["coin"].count + "/" + (COIN_NUMBER + (ENEMYINTENSITY * 2)), "50px Bradley Hand ITC", 185, 60);    

    if (endScreenOn) {
        drawObject("boss-bar",30, 60, 0.35, 0.15);
    }
...
function checkForHurt(image) {
    if (characterIsHurt) {
        ...
        animateGraphic(graphics.dead, 2);
    }
    return image;
}
